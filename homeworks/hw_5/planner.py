...
ПРАВИЛА:
1. Если для ответа на подвопрос требуется арифметика (разность, отношение, процентное изменение)
  или любое производное число – ОБЯЗАТЕЛЬНО добавь отдельный подвопрос с expected_tools=["calculate"].
  Он должен зависеть (depends_on) от подвопросов, которые дают исходные числа.
  Никогда не вычисляй в уме – доверь это инструменту calculate.
