...
CASES = [
    ...
    {
        "id": "Q4",
        "query": "Какова средняя инфляция за последние 3 года (2023, 2024, 2025)?",
        "comment": "Требует get_inflation за несколько лет и calculate для среднего.",
        "expected_tools_pwc": {"get_inflation", "calculate"},
        "must_have_keywords": ["%", "сред"],
        "forbid_hallucinated_tools": True,
    },
    {
        "id": "Q5",
        "query": "Сравни курс USD, EUR и CNY на сегодня",
    ...
    {
        "id": "Q6",
        "query": "Какова реальная доходность вклада при текущей ключевой ставке за вычетом последней доступной инфляции?",
        "comment": "Лично интересный макро-вопрос.",
        "expected_tools_pwc": {"get_key_rate", "get_inflation", "calculate"},
        "must_have_keywords": ["%", "реальн"],
        "forbid_hallucinated_tools": True,
    },
]
...
def run_case(case: dict, *, n: int = 5) -> dict:
    ...
    for i in range(n):
        ...
        try:
            r3 = run_pwc(case["query"], max_iter=3, verbose=False, validate=True)   # <-- добавить
        except Exception as e:
            r3 = {"answer": None, "error": f"{type(e).__name__}: {e}", "trace": [], "plan": None}
        check3 = _check_pwc(case, r3)
        pwc_validated["runs"].append(check3)
        pwc_validated["pass"] += int(check3["ok"])
...
def main():
    ...
    for r in results:
        print(f"  {r['id']}: single {r['single']['pass']}/{n}  "
              f"pwc {r['pwc']['pass']}/{n}  "
              f"pwc+валидатор {r['pwc_validated']['pass']}/{n}  — {r['query'][:60]}")
