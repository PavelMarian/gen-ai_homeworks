...
# Проверь ПОШАГОВО:
#
# 1. **Арифметика без calculate**: Если в любом ответе есть результат вычислений (разность, отношение, произведение, среднее, процентное изменение), но в `used_tools` этого подвопроса **нет** `calculate` – это БРАК. Даже если ответ выглядит логичным.
#
# 2. **Выдуманные числа**: Если инструмент не мог вернуть такие данные (например, `get_inflation` за будущий месяц, `get_fx_rate` за нерабочую дату), или число явно выходит за разумные пределы (инфляция > 100%, курс валюты < 0), – это БРАК.
#
# 3. **Согласованность данных**: Если подвопрос зависит от другого (`depends_on`), то числа в ответе должны быть согласованы с предыдущими ответами (например, курс EUR не должен противоречить курсу USD). Если согласованность нарушена – БРАК.
#
# 4. **Полнота плана**: Проверь, покрывает ли план ВСЕ аспекты исходного вопроса. Если какая-то часть вопроса осталась без ответа – это **replan**. Например, для реальной ставки нужны и ключевая ставка, и инфляция – если инфляции нет, это replan.
#
# 5. **Ошибки в ответах**: Если в ответе есть `(ошибка: ...)` или `error` – это автоматический БРАК.
#
# 6. **Недопустимые инструменты**: Если в `used_tools` или `expected_tools` есть инструмент, которого нет в списке допустимых (`get_fx_rate, get_key_rate, get_inflation, calculate`) – это БРАК (галлюцинация инструмента).
#
# **Важно**: Если есть хотя бы одно нарушение – вердикт `ok=False`.
# Если проблема в конкретных подвопросах – укажи их id в `rework_ids`.
# Если проблема в самом плане – `action=replan`.
#
# Вердикт:
# - ok=True, action=accept – если всё чисто.
# - ok=False, action=rework, rework_ids=[X] – если конкретные подвопросы нужно переделать.
# - ok=False, action=replan – если план в корне не охватывает вопрос.
# """
